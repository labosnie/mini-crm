...
def facture_update(request, pk):
    ...
    return render(
        request, "factures/facture_form.html", {"form": form, "facture": facture}
    )
...
def export_csv(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="factures_{datetime.now().strftime("%Y%m%d")}.csv"'
    )
...
def export_pdf(request):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="factures_{datetime.now().strftime("%Y%m%d")}.pdf"'
    )
...
def update_statut_facture(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Statut de la facture mis à jour : {facture.get_statut_paiement_display()}",
            )
        else:
