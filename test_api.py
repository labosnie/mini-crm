...
def test_clients(token):
    ...
    if response.status_code == 201:
        ...
        response = requests.patch(f"{BASE_URL}/clients/{client_id}/", 
...
def test_projets(token, client_id):
    ...
    response = requests.post(f"{BASE_URL}/projets/", 
                           json=projet_data, headers=headers)
...
def test_factures(token, client_id, projet_id):
    ...
    response = requests.post(f"{BASE_URL}/factures/", 
                           json=facture_data, headers=headers)
    ...
    if response.status_code == 201:
        ...
        response = requests.patch(f"{BASE_URL}/factures/{facture_id}/update_statut/", 
                                json=update_data, headers=headers)
